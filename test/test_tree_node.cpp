...
SCENARIO("Traversing a tree") {
  ...
  GIVEN("An internal node") {
    ...
    StoredInteriorNode<int> patient(
        {new StoredTerminalNode<int>(xLeftVal), new StoredTerminalNode<int>(xRightVal)},
        [&sum](int childValue) {
  ...
  GIVEN("A larger tree") {
    double prod = 1.0;
    std::function<double(double)> combiner = [&prod](double childValue) {
      return prod *= childValue;
    };
    ...
    static_cast<StoredInteriorNode<double>*>(immediateChildren[0])->setChildren(terminals1);
    static_cast<StoredInteriorNode<double>*>(immediateChildren[1])->setChildren(terminals2);
    static_cast<StoredInteriorNode<double>*>(immediateChildren[2])->setChildren(terminals3);

    double sum = 0.0;
    StoredInteriorNode<double> patient(immediateChildren, [&sum](double childValue) {
      return sum += childValue;
    });

    THEN("It returns its value") {
      REQUIRE(patient.value() == (pow(0.2, 5) + pow(0.2, 5) * pow(0.57, 2) + pow(0.2, 5) * pow(0.57, 2) * pow(0.94,3)));
    }
    THEN("It reports that it is not a terminal node") {
      REQUIRE(!patient.isTerminal());
    }
